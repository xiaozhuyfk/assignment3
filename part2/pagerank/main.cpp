...
void print_usage() {
    std::cerr
            << "Usage: ./pr_dist <graph_type> <vertices-per-node> <max-edges-per-vertex> [silent]\n";
    std::cerr << "Available graph types:\n";
    std::cerr << "\t-- uniform_random: A uniform random graph\n";
    std::cerr << "\t-- grid: A grid graph\n";
    std::cerr
            << "\t\t -> NOTE: vertices-per-node and number of processes should be perfect squares\n";
        ...
        for (int i = 0; i < NUM_RUNS; ++i) {
            ...
            if (world_rank == MASTER) {
                if (mismatch != graph.vertices_per_process) {
                    ...
                    std::cerr << "Mismatch at vertex " << mismatch_vertex << ": Got "
                            << mismatch_val << ", expected " <<
                            expected_val << "\n";
                }
            }
